...
<head lang="en">
  ...
  <script type="text/javascript">   
     $(function(){ //等效于$(document).ready(function(){})或者 jQuery(function($){ })   ，防止文档在完全加载（就绪）之前运行 jQuery 代码
	     ...
	     usercheck();	         
	     create_video_imglist();
	     videoplay();
	   }
   )
   
   function create_video_imglist(){
    	 var doc=new Array();
    	 
    	 doc[0] = create_img_tables("发布时间：2017-06-12","“领跑世界速度的超级计算机“","img/product_img/ANSYS-Fluids/流体分析.jpg",
	    	     "",
	    	     "click001()",Math.round(Math.random()*1000)+"+",Math.round(Math.random()*1000)+"+",Math.round(Math.random()*1000)+"+",
	    	     "video/170405.mp4"); 
    	 doc[1] = create_img_tables("发布时间：2017-07-13","“神威·太湖之光再次蝉联超算世界冠军”","img/product_img/ANSYS-Fluids/流体分析2.jpg",
	    	     "",
	    	     "click001()",Math.round(Math.random()*1000)+"+",Math.round(Math.random()*1000)+"+",Math.round(Math.random()*1000)+"+",
	    	     "video/170620.mp4"); 
    	 doc[2] = create_img_tables("发布时间：2017-08-12","“CAE对工程机械进行运动学仿真”","img/product_img/ANSYS-Structures/结构分析.jpg",
	    	     "",
	    	     "click001()",Math.round(Math.random()*1000)+"+",Math.round(Math.random()*1000)+"+",Math.round(Math.random()*1000)+"+",
	    	     "video/CAE对工程机械进行运动学仿真/wajueji.mp4"); 
    	 
	     for(var i=3;i<12;i++){
	       doc[i] = create_img_tables("发布时间：2016-09-12","“领跑世界速度的超级计算机","img/product_img/ANSYS-Structures/结构分析2.jpg",
	     ...
	     for(x in doc){
	    	 $("#table_imgs").append(doc[x]);
		 }
	     //alert(testindex.html());
	     
	     var testindex = create_img_index();
	     $("#table_imgs").append(testindex);
  	   }
  
   function create_img_tables(date,title,imgurl,txt,buttonfunction,qqnum,weixinnum,githubnum,videourl){
    	
		var divtop =     $("<div class=\"am-u-sm-12 am-u-md-6 am-u-lg-3\">");
		var divcontent = $("<div class=\"tpl-table-images-content\">");
		
	    var contentdetail = new Array();
		contentdetail[0]=$("<div class=\"tpl-table-images-content-i-time\">"+date+"</div>");
		//contentdetail[1]=$("<div class=\"tpl-i-title\">"+title+" </div>");
		
		contentdetail[2]=$("<a videohref=\""+videourl+"\"  class=\"video_link tpl-table-images-content-i\">");
				//var content_div=$("<div class=\"tpl-table-images-content-i-info\">");
				//var conetnt_user_img=$("<span class=\"ico\"> <img src=\"assets/img/user02.png\" alt=\"\">追逐  </span>");
				var ttext=$("<div class=\"tpl-i-title\">"+title+" </div>");
				var content2_span=$("<span class=\"tpl-table-images-content-i-shadow\"></span>");
				var content2_img=$("<img src=\""+imgurl+"\" alt=\"\">");

				contentdetail[2].append(ttext);
				contentdetail[2].append(content2_span);
				contentdetail[2].append(content2_img);
						
		contentdetail[3]=$("<div class=\"tpl-table-images-content-block\">");
				var content3_txt=$("<div class=\"tpl-i-font\">"+txt+"  </div>");
				var content3_more=$("<div class=\"tpl-i-more\">");
						var ul = $("<ul>");
						var li = new Array();
						li[0]=$("<li><span class= \"am-icon-qq am-text-warning\"> "+qqnum+"</span></li>");
						li[1]=$("<li><span class=\"am-icon-weixin am-text-success\"> "+weixinnum+"</span></li>");
						li[2]=$("<li><span class=\"am-icon-github font-green\">"+githubnum+"</span></li>");
						for(x in li){
							ul.append(li[x]);
						}
						content3_more.append(ul);
						
				var content3_download=$("<div class=\"am-btn-toolbar center\">");
						var btngroup = $("<div class=\"am-btn-group am-btn-group-xs tpl-edit-content-btn \">");
						var button = $("<button type=\"button\" class=\"am-btn am-btn-default am-btn-success\" "+
								   " onclick=\""+ buttonfunction +"\"> "+
								   " <span class=\"am-icon-plus\"></span> 下载 </button>");
						btngroup.append(button);	
						content3_download.append(btngroup);	
						
				contentdetail[3].append(content3_txt);				
				contentdetail[3].append(content3_more);				
				//contentdetail[3].append(content3_download);	
		for(x in contentdetail){
			divcontent.append(contentdetail[x]);	
			//alert(contentdetail[x]);					
		}	
		divtop.append(divcontent);
		return divtop;

   } 

     function create_img_index(){
    	 var divtop =     $("<div class=\"am-u-lg-12\">");
 		 var divcontent = $("<div class=\"am-cf\">");
 		 var divfr =      $("<div class=\"am-fr\">");
 		 var ul = $("<ul class=\"am-pagination tpl-pagination\">");
 		 var li = new Array();
 		 	li[0]=$("<li class=\"am-disabled\"><a href=\"#\">«</a></li>");
 			li[1]=$("<li class=\"am-active\"><a href=\"#\">1</a></li>");
 			for (var i=0;i<4;i++)
 			{
 				li[i+2]=$("<li><a href=\"#\">"+(i+2)+"</a></li>");
  			}
 			li[6]=$("<li><a href=\"#\">»</a></li>");
 		for(x in li){
 	 		ul.append(li[x]);
	 ...
	 function videoplay(){
		 ...
		 $(".video_link").click(function() {
			    ...
			    videojs("my-video", {}, function() {	
			        ...
			        myPlayer.play();
			        $("#myvideoname").html(name);
			        //$("#myvideocontent").html(name);
			    });
			    //$(".click-modal").click();
			});
...
<div class="toppic">
	 ...
	 <div class="toppic-title left">
			...
			<span class="toppic-title-span">CAE视频库</span>
			<p>CAE Video</p>
		</div>
		<div class="right toppic-progress">
			...
			<span><a href="solutions-knowledge.php" class="w-white">知识云</a></span>
						<i class=" am-icon-arrow-circle-right w-white"></i>
			<span><a href="solutions-knowledge-video.php" class="w-white">CAE视频库</a></span>
		</div>
	</div>
</div>
...
<div class=" am-container-1">
	<div class="part-title part-title-mar">
			...
			<span class="part-title-span">CAE视频库</span>
			<p>CAE Video database</p>
			</a>
	</div>
	<div class="solution-video-space" style="">
	           <div class="am-u-sm-12 am-u-md-12 am-u-lg-9 solution-tabs-img">
            			<video id="my-video" class="video-js" controls preload="auto"   width="800" height="480"
						  poster="#" data-setup="{}">   <!-- http://video-js.zencoder.com/oceans-clip.png -->
            	...
            	<div class="am-u-sm-12 am-u-md-12 am-u-lg-3  list" >
            			<div>视频介绍：<P class="" id="myvideoname">领跑世界速度的超级计算机</P> </div>
            			<!-- <div>视频内容：<P class="" id="myvideocontent">领跑世界速度的超级计算机</P> </div> -->
            			<!-- <div>相关内容：<P class="">...</P> </div>  -->
    ...
    <div class="am-g">
                    <div class="tpl-table-images" id="table_imgs">
                    <!--         <div class="am-u-sm-12 am-u-md-6 am-u-lg-4">
                                <div class="tpl-table-images-content">
                                    ...
                                    <a videohref="video/170405.mp4"  class="video_link tpl-table-images-content-i">
